...
│ ──────────────────────────────────────────────────────── │
│ 순서  질문                  분류      상태    작성자     │
│  10  렌더링은 얼마나…      영상제작  게시중   김관리     │
│  20  음성을 바꿀 수…       영상제작  게시중   김관리     │
│  30  프로젝트를 삭제…      프로젝트  게시중   이운영     │
│  99  결제는 어떻게…        기타      임시저장 김관리     │
│ ──────────────────────────────────────────────────────── │
│                       총 4건   ‹ 1 ›                     │
└──────────────────────────────────────────────────────────┘
     ↓ 행 클릭 / [+ 새 FAQ]
...
둘째, 첫 컬럼에 `badgedSeqColumn` 대신 `sort_order` 값을 그대로 보여준다. 관리자가 조정해야 하는 값이라 목록에서 바로 보이지 않으면 순서를 바꿀 때마다 모달을 열어 확인해야 한다. 모달의 도움말에는 `10, 20, 30`처럼 띄워 쓰면 사이에 끼워 넣기 쉽다고 안내한다.
