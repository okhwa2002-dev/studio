...
async def test_register_creates_pending_user(client):
    resp = await client.post(
        "/api/auth/register",
        json={"email": "new@example.com", "password": "pw123456", "name": "홍길동"},
    )
...
async def test_register_rejects_duplicate_email(client):
    await client.post(
        "/api/auth/register",
        json={"email": "dup@example.com", "password": "pw123456", "name": "홍길동"},
    )
    resp = await client.post(
        "/api/auth/register",
        json={"email": "dup@example.com", "password": "other-pw1", "name": "홍길동"},
    )
...
async def test_register_rejects_duplicate_email_differing_in_case_and_whitespace(client):
    ...
    resp1 = await client.post(
        "/api/auth/register",
        json={"email": "Race@Example.com", "password": "pw123456", "name": "홍길동"},
    )
    assert resp1.status_code == 201

    resp2 = await client.post(
        "/api/auth/register",
        json={"email": " race@example.com ", "password": "other-pw1", "name": "홍길동"},
    )
...
async def test_register_stores_name(client, db_session):
    ...
    resp = await client.post(
        "/api/auth/register",
        json={"email": "named@example.com", "password": "pw123456", "name": "홍길동"},
    )
    assert resp.status_code == 201
...
async def test_register_trims_name(client, db_session):
    ...
    await client.post(
        "/api/auth/register",
        json={"email": "trim@example.com", "password": "pw123456", "name": "  김철수  "},
    )
    row = await db_session.execute(
...
async def test_register_rejects_blank_name(client):
    resp = await client.post(
        "/api/auth/register",
        json={"email": "blank@example.com", "password": "pw123456", "name": "   "},
    )
...
async def test_register_rejects_too_long_name(client):
    resp = await client.post(
        "/api/auth/register",
        json={"email": "long@example.com", "password": "pw123456", "name": "가" * 51},
    )
    assert resp.status_code == 400
    assert resp.json()["code"] == "INVALID_NAME"
